...
abstract class Class118 {
   ...
   static jString aClass94_1617 = jString.createJString(")1a2)1m");
   ...
   static jString aClass94_1622 = jString.createJString("Card:");

   static final jString method1723(boolean var1, int var2, int var3) {
	   if(var2 >= 2 && 36 >= var2) {
           int var4 = 1;
   ...
   static final void method1724(int var0, int var1, int var2, int var3, int var4, int var5, int var7) {
	   /*
	    * 		if (x < 128 || y < 128 || x > 13056 || y > 13056) {
			spriteDrawX = -1;
			spriteDrawY = -1;
		} else {
			int z = getTileHeight(x, y, plane) - offset;
			z -= renderZ;
			x -= renderX;
			y -= renderY;
			int sineP = LDModel.sine[renderPitch];
			int cosineP = LDModel.cosine[renderPitch];
			int sizeY = LDModel.sine[renderYaw];
			int cosineY = LDModel.cosine[renderYaw];
			...
			z = calc;
			if (y >= 50) {
				spriteDrawY = (z << 9) / y + 167;
				spriteDrawX = (x << 9) / y + 256;
			} else {
				spriteDrawX = -1;
				spriteDrawY = -1;
			}
	    ...
	    */
         if(var5 >= 128 && ~var2 <= -129 && -13057 <= ~var5 && ~var2 >= -13057) {
            int var8 = Class121.method1736(Class26.plane, 1, var5, var2) + -var3;
            var2 -= Class77.renderY;
            var8 -= Class7.renderZ;
            var5 -= Npc.renderX;
            int sineP = Rasterizer.sineTable[Class139.renderPitch];
            int cosineP = Rasterizer.cosineTable[Class139.renderPitch];
            int sineY = Rasterizer.sineTable[Class3_Sub13_Sub25.renderYaw];
            int cosineY = Rasterizer.cosineTable[Class3_Sub13_Sub25.renderYaw];
            int calc = var5 * cosineY + sineY * var2 >> 16;
            var2 = var2 * cosineY + -(sineY * var5) >> 16;
            var5 = calc;
            calc = cosineP * var8 - var2 * sineP >> 16;
               var2 = var2 * cosineP + var8 * sineP >> 16;
               if(50 > var2) {
                  ClickMaskNode.anInt2208 = -1;
                  Class32.anInt590 = -1;
               } else if(!HDToolkit.openGLEnabled) {
                  Class32.anInt590 = (var5 << 9) / var2 + var0;
                  ClickMaskNode.anInt2208 = (calc << 9) / var2 + var4;
               } else {
                  int var14 = var1 * 512 >> 8;
                  Class32.anInt590 = var14 * var5 / var2 + var0;
                  int var15 = 512 * var7 >> 8;
                  ClickMaskNode.anInt2208 = var15 * calc / var2 + var4;
               }
   ...
   abstract AbstractObjectCache method1725(AbstractObjectCache var1);
   ...
   public static void method1726() {
	...
	static final int method1727() {
		return KeyboardHandler.aClass93_1911.size();
	}
